@if (is_user_logged_in())
  <a class="btn btn-green" href="/my-account/orders" title="{{ _e('My Orders', 'rastatheme') }}">{{ _e('Orders', 'rastatheme') }}</a>
  <a class="btn btn-gold" href="{!! get_permalink( get_option('woocommerce_myaccount_page_id') ) !!}" title="{{ _e('My Account','rastatheme') }}">{{ _e('Account','rastatheme') }} </a>
  <a class="btn btn-red" href="{!! wp_logout_url( $_SERVER['REQUEST_URI'] ) !!}" title="{{ _e('Logout', 'rastatheme') }}">{{ _e('Logout', 'rastatheme') }}</a>
@else
